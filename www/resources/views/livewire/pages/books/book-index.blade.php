<div class="w-full flex flex-col gap-4 p-3" x-on:close-modal.window="$flux.modal($event.detail.name).close()">
    ...
    <div class="flex flex-col gap-4">
        <div class="flex gap-4">
            ...
            <flux:modal.trigger name="create-book">
                <flux:button variant="primary" icon="folder-plus" color="amber"
                    class="hover:opacity-90 transition shrink-0" x-on:click="$flux.modal('create-book').show()">
                    Add Book
                </flux:button>
            </flux:modal.trigger>
        ...
        <flux:table>
            ...
            <flux:table.rows>
                @forelse($books as $book)
                    ...
                    {{-- Edit modal lives inside the loop so $book is always defined --}}
    ...
    <x-confirm-modal name="archive-book" title="Archive Book"
        description="This will move the book to the archives. Continue?" confirm-label="Archive"
        confirm-variant="danger" confirm-action="archiveConfirmed" />

    <flux:modal name="create-book"
        class="flex flex-col gap-4 p-4 rounded-lg bg-white dark:bg-zinc-800 border border-solid border-zinc-600">
